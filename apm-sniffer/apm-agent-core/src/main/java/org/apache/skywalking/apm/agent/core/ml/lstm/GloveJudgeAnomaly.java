...
* Time Test
* */
public class GloveJudgeAnomaly {
    ...
    GloveWordEmbeddingDict gloveWordEmbeddingDict = new GloveWordEmbeddingDict();
    Predictor<NDList, NDList> predictor;
    String isFake;

    public GloveJudgeAnomaly() {
        try {
            gloveWordEmbeddingDict.initEmbeddingDict();
            ...
            LOGGER.error("GloveJudgeAnomaly init exception:" + e);
            System.exit(1);
        }
    }
    ...
    private NDList infer(String[] logs, int logCount) throws IOException, ModelNotFoundException, MalformedModelException, TranslateException {
        ...
        try (NDManager ndManager = NDManager.newBaseManager()) {
            NDArray logsEmbedding = ndManager.create(new Shape(logCount, 50));
            long t1 = System.nanoTime();

            for (int i = 0; i < logs.length; i++) {
                String errorLog = logs[i];
                NDArray OneLogEmbedding = gloveWordEmbeddingDict.Log2Embedding(errorLog);
                logsEmbedding.set(new NDIndex(i), OneLogEmbedding);
            }
    ...
    public GloveJudgeAnomaly(String isFake) {
        this.isFake = isFake;
    }
